...
Add a softmax function to transform model outputs into probability values between 0 and 1. Implement a final function to determine the most likely image classification.

First, include the `Jimp` and `Lodash` library source files in `main.js`'s head tag.
...
Then, add the necessary functions to `main.js`.
