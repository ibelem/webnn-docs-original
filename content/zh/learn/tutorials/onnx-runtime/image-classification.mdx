...
- 操作系统：Windows（推荐最新版本）
- 浏览器：[Microsoft Edge Canary](https://www.microsoft.com/en-us/edge/download/insider?form=MA13FJ)
- 硬件：支持 WebNN 的 GPU 和 NPU，并具有最新的驱动程序

### 在 Edge Canary 中启用 WebNN API

1. 下载并安装 [Microsoft Edge Canary](https://www.microsoft.com/en-us/edge/download/insider?form=MA13FJ)
2. 启动 Edge Canary，在地址栏中导航到 `about:flags`
...
2. 通过点击 VSCode 右下角的 Go Live 按钮验证设置，这将在 Edge Canary 中启动本地服务器。
3. 创建一个新的 `main.js` 文件，用于存放应用的 JavaScript 代码。
4. 在项目目录中创建 `images` 子文件夹，并添加一个图像文件（在本演示中，将其命名为 `image.jpg`）。
5. 从 ONNX 模型仓库下载 [`mobilenet2-10.onnx`](https://github.com/onnx/models/blob/main/validated/vision/classification/mobilenet/model/mobilenetv2-10.onnx) 模型，并将其保存在项目的根文件夹中。
...
async function classifyImage(pathToImage){ 
  var imageTensor = await getImageTensorFromPath(pathToImage); // 将图像转换为张量
  var predictions = await runModel(imageTensor); // 在张量上运行推理
...
```js showLineNumbers
 async function getImageTensorFromPath(path, width = 224, height = 224) {
  var image = await loadImagefromPath(path, width, height); // 1. 加载图像
  var imageTensor = imageDataToTensor(image); // 2. 转换为张量
  return imageTensor; // 3. 返回张量
} 

async function loadImagefromPath(path, resizedWidth, resizedHeight) {
  var imageData = await Jimp.read(path).then(imageBuffer => { // 使用 Jimp 加载图像并调整大小
