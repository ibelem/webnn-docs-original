...
This table documents the ONNX operators supported by WebNN EP in ONNX Runtime Web. The Opset column shows supported versions in the format `7-12, 13+`, which indicates support for versions 7 through 12, plus version 13 and above.

> <InfoIcon /> ONNX Runtime guarantees support only for models using opset version 7 or higher in the 'ai.onnx' domain.
